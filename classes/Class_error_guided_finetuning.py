"""
This script is used to define the class for error-guided finetuning including hyperparameters, loss functions,
training and testing. Specifically, we define functions which aim at getting the probability maps from error maps.

During experiments, we found that the performance without joint training stage is better in average.
So we use results of the second stage as the final results.
You can try the third stage for your test scenes, maybe it could get better results.
"""
import os
import time
import random
...
from configs.Config_error_guided_finetuning import Config
from tools.dataloader_for_error_guided_finetuning import *

def get_cur_time():
    return time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())

def load_params(src_dict, des_model):
    # src_dict: state dict of source models
    # des_model: models of the destination models
...
class Error_guided_finetuning_SingleTargetView:
    # define the counters
    ...
    def __init__(self, lf_name, mat_path, conf=Config(), ext_model="", save_name="", save_prefix="", repeat_iter=0, gpu_num=2):
        ...
        # Set dataloader
        self.dataloader_align = DataloaderForAlignNetWithBatch_ErrorGuide(mat_path=mat_path,
                                                                          refPos=self.refPos,
                                                                          scale=self.scale,
                                                                          view_num=self.view_num,
                                                                          patch_size=self.conf.align_patch_size,
                                                                          batch_size=self.conf.align_batch_size)
        self.dataloader_aggre = DataloaderForLFZSSRWithBatch_ErrorGuide(mat_path=mat_path,
                                                                        refPos=self.refPos,
                                                                        scale=self.scale,
                                                                        view_num=self.view_num,
                                                                        patch_size=self.patch_size,
                                                                        batch_size=self.conf.batch_size)
        self.dataloader_ft = DataloaderForLFZSSRWithBatch_ErrorGuide(mat_path=mat_path,
                                                                     refPos=self.refPos,
                                                                     scale=self.scale,
                                                                     view_num=self.view_num,
                                                                     patch_size=self.patch_size,
        ...
        if self.conf.record:
            ...
            self.writer = SummaryWriter(log_dir="{}/logs/{}/".format(self.save_prefix, self.save_name),
                                        comment="LF-WarpFusionNet internal curve with {}".format(self.save_name))
        ...
        self.align_aggre_net_test = AlignWithAggreNet_ForTest(refPos=self.refPos, scale=self.scale,
                                                              ...
                                                              pad_size=self.conf.pad_size,
                                                              view_num=self.conf.view_num)
        self.ext_dict = torch.load(self.ext_model)["model_dict"]

        self.align_net = load_params(self.ext_dict, self.align_net)
        self.align_aggre_net = load_params(self.ext_dict, self.align_aggre_net)
        ...
        self.align_aggre_optimizer = optim.Adam(filter(lambda p: p.requires_grad,
    ...
    #### ---- Define test for AlignNet and get the probability map.
    ...
    def test_AlignNet(self):
        ...
        # calculate the align mse of LR resolution (not used in training)
        ...
        # calculate the LR error map
        warp_error_map = (warped_vdsr_lf - vdsr_cv).abs() # [1, UV, H, W]
        warp_error_map = torch.sum(warp_error_map, dim=1, keepdim=True)
        lr_error_map = self.resize(warp_error_map, 1.0 / self.scale) # [1, 1, h, w]
        lr_error_map = lr_error_map.abs()
        lr_error_map = lr_error_map.squeeze()
        lr_error_map = lr_error_map.cpu().data.numpy()

        lr_error_map = lr_error_map / (np.sum(lr_error_map)) # [h, w] and normalized by the sum of this map

        prob_map_total = lr_error_map.flatten() / lr_error_map.sum() if lr_error_map.sum() != 0 else np.ones_like(lr_error_map.flatten()) / lr_error_map.flatten().shape[0]
        ...
        return test_mse, prob_map_total

    #### ---- Define QuickTest for probability map and Test for final inference.

    def quick_test_Align_Aggre(self, mode):
        # mode could be "aggre" or "ft"
        self.align_aggre_net.eval()

        lr_lf = self.dataloader_aggre.lf_lr
        lr_lf = lf_modcrop(lr_lf, self.scale)
        lr_father_cv = lr_lf[self.refPos[0], self.refPos[1]] # [h, w]
        U = lr_lf.shape[0]
        # get the central light fields
        view_start = (U - self.view_num) // 2
        lr_lf = lr_lf[view_start: view_start + self.view_num, view_start: view_start + self.view_num]
        lr_lf = lr_lf.astype(float) / 255.0
        lr_lf = torch.Tensor(lr_lf).float().view(1, -1, lr_lf.shape[2], lr_lf.shape[3])
        lr_father_cv = torch.Tensor(lr_father_cv.astype(float) / 255.0).unsqueeze(0).unsqueeze(0)  # [1, 1, h, w]
        # feed into devices
        if self.conf.use_cuda:
            lr_lf = lr_lf.cuda()
            lr_father_cv = lr_father_cv.cuda()
        else:
            lr_lf = lr_lf.cpu()
            lr_father_cv = lr_father_cv.cpu()
        ### --- Downsample
        lr_lf_son = self.resize(lr_lf, 1.0 / self.scale)
        aug_scale = 1.0
        ### --- Copy weights
        ...
        with torch.no_grad():
            try:
                _, _, sr_res, _, _ = self.align_aggre_net_test(lr_lf_son, aug_scale, self.conf.set_name)
            except RuntimeError as exception:
                if "out of memory" in str(exception):
                    print("WARNING: out of memory")
                    if hasattr(torch.cuda, 'empty_cache'):
                        torch.cuda.empty_cache()
                        _, _, sr_res, _, _ = self.align_aggre_net_test(lr_lf_son, aug_scale, self.conf.set_name) # [1, 1, h, w]
                else:
                    raise exception
        # calculate the warp mse of LR resolution (not used in training)

        test_mse = self.L2criterion(sr_res, lr_father_cv)
        test_mse = test_mse.cpu().data.numpy()
        print("Align-AggreNet Quick test mse: {:.10f}".format(test_mse))

        lr_error_map = (sr_res - lr_father_cv).abs()
        lr_error_map = lr_error_map.squeeze()
        lr_error_map = lr_error_map.cpu().data.numpy()
        lr_error_map = lr_error_map / (np.sum(lr_error_map))
        ...
        if self.conf.record:
            if mode == "aggre":
                self.writer.add_scalar("scalar_aggre_stage/Quick_test_mse", test_mse, self.aggre_iter)
            elif mode == "ft":
                self.writer.add_scalar("scalar_ft_stage/Quick_test_mse", test_mse, self.ft_iter)
            else:
    ...
    def trainer_align_stage(self):
        ...
        # test the init models
        test_mse, prob_map_total = self.test_AlignNet()
        ...
        for self.align_iter in range(1, self.conf.max_iters):
            ...
            lf_patch = self.dataloader_align.get_patch(prob_map_total)
            aug_scale = self.scale
            if self.conf.use_cuda:
            ...
            # learning rate policy and early stop checking
            ...
            if self.align_iter == (self.conf.max_iters - 1):
                ...
                if self.conf.train_time_test:
                    test_mse, _ = self.test_AlignNet()
                return
    ...
    def trainer_aggre_stage(self):
        ...
        # test the init models
        PSNR_value, sr_res = self.test(mode="aggre")
        self.aggre_test_PSNRs.append(PSNR_value)
        self.aggre_test_iters.append(self.aggre_iter // self.conf.test_step)
        MSE_value, prob_map_total = self.quick_test_Align_Aggre(mode="aggre")
        self.init_quick_sr_error_map = prob_map_total
        ...
        for self.aggre_iter in range(1, self.conf.max_iters):
            ...
            # change the learning rate if it's necessary
            # for param_group in self.warp_fusion_optimizer.param_groups:
            #     param_group["lr"] = self.lr_fusion_stage

            # get patches
            lr_patch, hr_patch, _, hr_ref_patch = self.dataloader_aggre.get_patch(prob_map_total)
            aug_scale = 1.0
            if self.conf.use_cuda:
            ...
            # forward
            self.align_aggre_optimizer.zero_grad()
            ...
            if self.conf.zssr_bp_ratio > 0.0:
                ...
                sr_loss.backward()
            self.align_aggre_optimizer.step()

            # record
            if self.conf.record:
                self.writer.add_scalar("scalar_aggre_stage/SR_loss", sr_loss.cpu().data, self.aggre_iter)
                if self.conf.zssr_bp_ratio > 0.0:
                    self.writer.add_scalar("scalar_aggre_stage/BP_loss", bp_loss.cpu().data, self.aggre_iter)
                    self.writer.add_scalar("scalar_aggre_stage/Total_loss", total_loss.cpu().data, self.aggre_iter)
            # display
                ...
                if self.conf.zssr_bp_ratio > 0.0:
                    print("===> {}: Iteration: {}, lr: {:.5f}, SR loss: {:.10f}, "
                          ...
                          format(get_cur_time(), self.aggre_iter,
                                 self.align_aggre_optimizer.param_groups[0]["lr"],
            ...
            # if self.aggre_iter == self.conf.align_aggre_iter_step:
            ...
            if self.aggre_iter == (self.conf.max_iter_aggre - 1):
                ...
                self.aggre_test_PSNRs.append(PSNR_value)
                self.aggre_test_iters.append(self.aggre_iter // self.conf.test_step)
                MSE_value, _ = self.quick_test_Align_Aggre(mode="aggre")
                return sr_res

    def ft_trainer(self):
        cudnn.benchmark = True
        # freeze VDSR
        self.align_aggre_net.module.set_zero_lr_VDSR()
        # test the init models
        print("Free AlignNet ------")
        ...
        MSE_value, prob_map_total = self.quick_test_Align_Aggre(mode="ft")
        ...
        for self.ft_iter in range(1, self.conf.max_iters):
            ...
            # get patches
            lr_patch, hr_patch, _, hr_ref_patch = self.dataloader_ft.get_patch(prob_map_total)
            aug_scale = 1.0
            if self.conf.use_cuda:
                lr_patch = lr_patch.cuda()
                hr_patch = hr_patch.cuda()
            ...
            # backward
            # for AlignNet
            align_loss = self.align_loss(warped_vdsr_lf, vdsr_ref)
            # for FusionNet
            if self.conf.zssr_bp_ratio > 0.0:
                range_spatial_x_hr_pad = torch.arange(0, patch_w + 2 * self.conf.pad_size).view(1, -1).repeat(self.gpu_num, 1)
                range_spatial_y_hr_pad = torch.arange(0, patch_h + 2 * self.conf.pad_size).view(1, -1).repeat(self.gpu_num, 1)
                ...
                bp_loss = 0.0
            sr_loss = self.L1criterion(ref_sr_res, hr_ref_patch)
            total_loss = sr_loss + align_loss * self.conf.align_loss_weight + bp_loss * self.conf.zssr_bp_ratio
            total_loss.backward()
            ...
            if self.conf.record:
                ...
                self.writer.add_scalar("scalar_ft_stage/Align_loss", align_loss.cpu().data, self.ft_iter)
                if self.conf.zssr_bp_ratio > 0.0:
            ...
            if (self.ft_iter % self.conf.display_loss_step == 0):
                if self.conf.zssr_bp_ratio > 0.0:
                    print(
                        ...
                        "Warp LR loss: {:.10f}, Total loss: {:.10f}".
            ...
            if self.ft_iter == (self.conf.max_iter_ft - 1):
                ...
                self.ft_test_PSNRs.append(PSNR_value)
                self.ft_test_iters.append(self.ft_iter // self.conf.test_step)
                MSE_value, _ = self.quick_test_Align_Aggre(mode="ft")
                return sr_res
    ...
    def final_test(self):
        ...
        for k in range(0, 16):
            ...
            # t_tmp_output = transfer_img_to_uint8(t_tmp_output)
    ...
    def run_three_stage(self):
        # print("Test the init models -------")
        # PSNR_init, _ = self.test(mode="aggre")
        ...
        print("Final testing for fusion stage ----------")
        ...
        results_dict = {"aggre_sr_res": aggre_trainer_sr_res,
                        ...
                        "psnr_aggre_sr": self.aggre_test_PSNRs[-1],
